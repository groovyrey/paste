--[[
	OrbyteUI.client.lua
	Standalone LocalScript — modern dark-themed UI built from ModernUI.client.lua.
	No require() needed.

	Builds a window titled "Orbyte" and shows whether the "Orbyte" folder was
	found or created. Uses the ModernUI theme, helpers, components and window
	behaviour in full; only the title and the "BUILD YOUR UI HERE" section differ.
--]]
...
-- Checks for the folder and creates it if missing.
...
local function refreshStatus()
	local success, message = checkFolder()
	statusLabel.Text = message
	statusLabel.TextColor3 = success and Theme.Text or Theme.AccentHover
end

refreshStatus()

AddButton("Recheck Folder", refreshStatus)
